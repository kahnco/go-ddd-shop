...
- **메시징**: NATS / JetStream (영속 스트림·내구 소비자, `NATS_JETSTREAM=1`)
- **저장소**: PostgreSQL
...
5. 영속 스트림 — JetStream 컨텍스트별 스트림과 내구 소비자(구독 전 발행 유실 해결, at-least-once)
6. 회원과 장바구니 — 회원 확인 → 담기 → 결제(checkout)로 기존 사가에 연결(동기 호출·ACL)
7. 읽기 모델(CQRS) 강화 — 주문 이벤트로 조회 전용 뷰 구축, "내 주문 목록"을 싸게
8. OpenTelemetry 분산추적 — 상관 ID를 trace/span으로, HTTP·이벤트 경계로 전파해 Jaeger 로 시각화
...
for s in ordering inventory payment shipping catalog customer cart readmodel; do
  docker build --build-arg SERVICE=$s -t go-ddd-shop/$s:part-24 .
  kind load docker-image go-ddd-shop/$s:part-24 --name shop
done
...
`/metrics`(Prometheus)에 RED·비즈니스·신뢰성 지표를 노출하고, **Prometheus + Grafana**
스택으로 대시보드를 자동 프로비저닝합니다(`deploy/k8s/prometheus.yaml`·`grafana.yaml`).

```bash
kubectl apply -f deploy/k8s/prometheus.yaml -f deploy/k8s/grafana.yaml
kubectl port-forward -n shop svc/grafana 3000:3000   # http://localhost:3000 (익명 Admin)
```

주요 지표: `http_requests_total`·`http_request_duration_seconds`(RED),
`orders_placed_total`·`order_revenue_won_total`(비즈니스), `dlq_messages_total`·
`events_consumed_total`(신뢰성). 대시보드 "Shop — RED · 비즈니스 · SLO" 가 함께 뜹니다.

**SLO(서비스 수준 목표)**

| SLO | 목표 | 측정(PromQL 개요) |
|---|---|---|
| 가용성 | 30일 99.9% | `1 - ((5xx 요청률 or vector(0)) / 전체 요청률)` |
| 지연 | p95 < 500ms | `histogram_quantile(0.95, …_bucket)` |
| 신뢰성 | DLQ 유입 ≈ 0 | `rate(dlq_messages_total[5m])` |

99.9% 는 30일 기준 **약 43분** 의 에러 예산(error budget)입니다. 예산을 빠르게 태우면
(번 레이트↑) 배포를 멈추고 안정화에 집중하는 게 SRE 의 기본 규율입니다.
